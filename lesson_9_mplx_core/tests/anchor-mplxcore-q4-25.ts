...
import { fetchAssetV1, MPL_CORE_PROGRAM_ID } from "@metaplex-foundation/mpl-core";
import { createUmi } from "@metaplex-foundation/umi-bundle-defaults";
import { publicKey as pk } from "@metaplex-foundation/umi";
...
describe("anchor-mplxcore-q4-25", () => {
  ...
  before(async () => {
    umi = createUmi('http://localhost:8899');
  ...
  describe("UpdateNft", () => {
    it("Update an NFT", async () => {
      const updatedName = "Updated NFTest";
      await program.methods
        .updateNft(updatedName)
        .accountsStrict({
          authority: creator.publicKey,
          asset: asset.publicKey,
          collection: collection.publicKey,
        ...
        const umiAssetPublicKey = pk(asset.publicKey.toBase58());
        const { name } = await fetchAssetV1(umi, umiAssetPublicKey);
        assert.equal(name, updatedName, "NFT name should be updated");
    });
  });
});